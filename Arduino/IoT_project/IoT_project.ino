...
// 강아지 바이탈사인
RTC_DATA_ATTR String healthStatus = "normal";  // 기본값: normal , 이상값 검출 시: abnormal
...
// 바이탈 사인 검사 시간
// 측정 월
RTC_DATA_ATTR int sensingMonth = 0;
...
RTC_DATA_ATTR int abnormalAlarmDay = 0;

// 오전 10시 , 오후 2시, 오후 5시, 오후 8시
const int abnormalHour[4] = {10, 14, 17, 20};
RTC_DATA_ATTR boolean abnormalAlarmStatus[4] = {false, false, false, false};
...
RTC_DATA_ATTR int foodAlarmDay[5] = {0, 0, 0, 0, 0};
RTC_DATA_ATTR int foodTime[5] = {-1, -1, -1, -1, -1};
RTC_DATA_ATTR boolean foodAlarmStatus[5] = {false, false, false, false, false};
RTC_DATA_ATTR int foodHour[5] = {-1, -1, -1, -1, -1};
RTC_DATA_ATTR int foodMin[5] = {-1, -1, -1, -1, -1};
...
void loop() {
  ...
  if ((sensingNow)) {
    ...
    // 테스트 코드
    Serial.println("실시간 센싱 결과 전송 시작");
  ...
  if (((sensingMode_Day1) && (sensingHour_1 == timeinfo.tm_hour)) || ((sensingMode_Day2) && (sensingHour_2 == timeinfo.tm_hour))) {
    ...
    if ((sensingPublishStatus_1) || (sensingPublishStatus_2)) {
      ...
      // test code
      Serial.println("주기적 센싱 결과 JSON으로 보내기 시작");
      ...
      while(true) {
        if (chichiShadow.publish(pTOPIC_NAME, payload) == 0) {
          ...
          // 테스트 코드
          Serial.println("주기적 센싱 결과 JSON 전송 완료");
          delay(1000);
          deepSleepFlag = false;
          break;
  ...
  if ((!sensingNow) && (deepSleepFlag)) {  // 실시간 측정 모드라면 딥슬립에 빠지면 안됨 (+ 딥슬립 flag)
    ...
    nowSec_d = timeinfo.tm_sec;
    nowTime_d = nowHour_d*3600 + nowMin_d*60 +nowSec_d;

    // 현재 시간을 제대로 받았는지 확인
    Serial.printf("now time:    %d : %d : %d \n", nowHour_d, nowMin_d, nowSec_d);
    ...
    // 비정상 시 주기적 알람 시간 비교
    if (healthStatus == "abnormal") {  // 강아지 상태가 비정상이면
      for (int j=0; j<4; j++) {
        int abnormalTime = abnormalHour[j] * 3600;
        
        if (closeTime > nowTime_d) {  // 최근접 알람 시간이 오늘일 때
          if (abnormalTime > nowTime_d) {  // 비정상 알람 시간도 오늘인 경우
            if (closeTime > abnormalTime) {  // 최근접 알람 시간보다 비정상 알람 시간이 빠르면
              closeTime = abnormalTime;  // 타이머 재설정
            }
          }
          
        } else if (closeTime <= nowTime_d) {  // 최근접 알람 시간이 내일일 때
          if (abnormalTime > nowTime_d) {  // 비정상 알람 시간은 오늘이면
            closeTime = abnormalTime;   // 타이머 재설정
            
          } else if (abnormalTime <= nowTime_d) {  // 비정상 알람 시간도 내일이면
            if (closeTime > abnormalTime) {  // 최근접 알람 시간보다 비정상 알람 시간이 빠르면
              closeTime = abnormalTime;  // 타이머 재설정
            }
      ...
      }  // 비정상 알람 시간 비교 for 문 끝
      
    }  // 강아지 상태가 비정상일 때 비정상 알람 시간 비교 if 문 끝
    
    // foodTime이 설정되었으면 비교
    for (int i=0; i<5; i++) {
      if (foodTime[i] < 0) {  // 미설정이면 비교 x
        continue;
      }
      
      int compTime_f = foodHour[i]*3600 + foodMin[i]*60;
      
      if (closeTime > nowTime_d) {  // 최근접 알람 시간이 오늘일 때
        if (compTime_f > nowTime_d) {  // 밥 알람 시간도 오늘이면
          if (closeTime > compTime_f) {  // 밥 알람 시간이 최근접 알람 시간보다 빠를 때
            closeTime = compTime_f;  // 타이머 재설정        
          }
        }
      } else if (closeTime <= nowTime_d) {  // 최근접 알람 시간이 내일일 때
        if (compTime_f > nowTime_d) {  // 밥 알람 시간은 오늘이면
          closeTime = compTime_f;  // 타이머 재설정
          
        } else if (compTime_f <= nowTime_d) {  // 밥 알람 시간도 내일이면
          if (closeTime > compTime_f) {  // 밥 알람 시간이 더 빠를 때
            closeTime = compTime_f;  // 타이머 재설정
          }
        }    
      }
    ...
    Serial.print(closeTime/3600);
    Serial.print(" : ");
    Serial.print(closeTime%3600/60);
    Serial.print(" : ");
    Serial.println(closeTime%3600%60);

    // wake up 이유 출력. 첫 번째에는 0번 뜨고 이후엔 타이머라고 떠야한다.
    print_wakeup_reason();
    ...
    // 딥슬립 들어간다고 미리 안내
    Serial.print("sleep this time: ");
    Serial.println(TIME_TO_SLEEP);
    Serial.println("Going to sleep now");
    Serial.flush();

    // 실시간 측정 모드를 위해 15번 핀 연결된 스위치를 누르면 wake up 하도록 설정
    esp_sleep_enable_ext0_wakeup(GPIO_NUM_15, 1);  // 1: high, 0: low)

    // 딥슬립 시작
    esp_deep_sleep_start();

    // 이 밑에가 출력되면 deep sleep 실패
    Serial.println("This will never be printed (failed to DeepSleep)");
    
  }  // 실시간 측정 모드가 아닐 때 딥슬립 if문 끝
